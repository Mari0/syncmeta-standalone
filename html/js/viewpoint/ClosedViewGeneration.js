/*global define, $, y*/
define(['lodash', 'graphlib', 'EntityManager'],
    function (_, graphlib, EntityManager) {
        /**
        ...
        function CVG(viewType) {
            ...
            _.forEach(neighborsOfOrigin, function (neighborId) {
                ...
                var newNodeId, viewtypes, viewType2;
                ...
                if (node.type === 'Node Shape' || node.type === 'Edge Shape' || node.type === 'Relation') {
                    ...
                    viewtypes = EntityManager.getNodesByType('ViewRelationship');
                    for (var key in viewtypes) {
                        if (viewtypes.hasOwnProperty(key)) {
                            viewType2 = viewtypes[key];
                            if (viewType2.getAttribute(viewType2.getEntityId() + '[target]').getValue().getValue() === neighborId)
                                newNodeId = viewType2.getEntityId();
                        }
                    }
                } else if (viewType.getType() === 'ViewRelationship' && node.type === 'Object') {
                    viewtypes = EntityManager.getNodesByType('ViewObject');
                    for (var key2 in viewtypes) {
                        if (viewtypes.hasOwnProperty(key2)) {
                            viewType2 = viewtypes[key2];
                            if (viewType2.getAttribute(viewType2.getEntityId() + '[target]').getValue().getValue() === neighborId)
                                newNodeId = viewType2.getEntityId();
                        }
                    }
