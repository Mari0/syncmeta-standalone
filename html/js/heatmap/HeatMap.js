...
define([
...
],function ($, Util, NodePreview, NodeAddOperation,NodeMoveOperation,NodeResizeOperation,NodeDeleteOperation, CanvasViewChangeOperation) {
    ...
    function HeatMap(){
        ...
        var $heatmap = $("#heatmap");
        var scaleFactorWidth = $heatmap.width() / 9000;
        var scaleFactorHeight = $('#heatmap-frame').height() / 9000;
        ...
        var addNodePreview = function(id, x, y, width, height, color){
            var nodePreview = new NodePreview(id, x, y, width, height, {width: scaleFactorWidth, height: scaleFactorHeight}, color);
        ...
        var updateWindow = function(viewChangeOperation){
            ...
            $window.css({
                top: -top * scaleFactorHeight / zoom,
                left: -left * scaleFactorWidth / zoom,
                width: width * scaleFactorWidth / (zoom),
                height: height * scaleFactorHeight / (zoom) 
            });
            $window.show();
        };
        ...
        var updateZoom = function(){
            ...
            var translateX = -(centerX - 4500) * scaleFactorWidth;
            var translatY =  -(centerY - 4500) * scaleFactorHeight;
        ...
        if(model){
            for (var nodeId in model.nodes) {
                if (model.nodes.hasOwnProperty(nodeId)) {
                    ...
                    var nodePreview = addNodePreview(nodeId, node.left, node.top, node.width, node.height, null);
